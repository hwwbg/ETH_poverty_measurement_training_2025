...
*==============================================================================	
	...
	* 1.1 check measurement and stunit
	...
	/*preserve
		keep if MEASURMENT == 99
		export excel using "${path}\issue\issue_general.xlsx", sheet("MEASURMENT") sheetmodify firstrow(variables) 
	restore*/
	

	* 1.2 check the stunit for different measurement
	* 1.2.1 Gram, the stunit should not be larger than 1000
	cap assert STUNIT <= 1000 if MEASURMENT == 1
	if (_rc){
		tab STUNIT if MEASURMENT == 1 //the regular STUNIT for Gram should be 1000, however they are quite a few obs have more than 10000 as STUNIT
		//ISSUE 3: extreme stunit
		/*preserve
			keep if MEASURMENT == 1 & STUNIT > 1000
	...
	* 1.2.2 Cubic Centermetter, the stunit should not be large than 1000
	cap assert STUNIT <= 1000 if MEASURMENT == 3
	if (_rc) {
		tab STUNIT if MEASURMENT == 3
		//ISSUE 4: extreme stunit
		/*preserve
			keep if MEASURMENT == 3 & STUNIT > 1000
			export excel using "${path}\issue\issue_general.xlsx", sheet("stunit_cc") sheetmodify firstrow(variables) 
		restore*/
	}
	
	* 1.2.3 kg
	cap assert STUNIT <= 1 if MEASURMENT == 20
	if (_rc) {
		tab STUNIT if MEASURMENT == 20
		//ISSUE 5: extreme stunit
		/*preserve
			keep if MEASURMENT == 20 & STUNIT > 1
	...
	* 1.2.4 number/services: should have stunit as 1
	cap assert STUNIT == 1 if MEASURMENT == 4     //STUNT is 1 when measurement is Number
	if (_rc) {
		tab STUNIT if MEASURMENT == 4
		//ISSUE 6: extreme stunit
		/*preserve
			keep if MEASURMENT == 4 & STUNIT != 1
			export excel using "${path}\issue\issue_general.xlsx", sheet("stunit_number") sheetmodify firstrow(variables) 
		restore*/
	}	
	
	
	assert STUNIT == 1 if MEASURMENT == 21    //STUNT is 1 when measurement is services
	if (_rc) {
		tab STUNIT if MEASURMENT == 21
		//ISSUE 7: extreme stunit
		/*preserve
			keep if MEASURMENT == 21 & STUNIT != 1
	...
	* 1.4 generate class
	gen class = 1 if MEASURMENT == 1 | MEASURMENT == 3 | MEASURMENT == 20
...
*==============================================================================
	...
	* 2.1 Large quantity
	...
	/*preserve
		...
		keep hhid hh_size major itemc TYPE SOURCE MEASURMENT ann_value ann_quant ann_quant_st
...
*******************************************************************************
	...
	* 3.3 construct framework dataset of each possible REP-item
	...
	duplicates drop
	count
	fillin itemc REP // rectangularize the data set
	sort itemc _fillin
	drop _fillin
	merge m:1 REP using `reg', assert(3) nogen
	...
	**  b. construct national price as weighted average of region-urban prices
...
*******************************************************************************
	**  i. clean up and save
	...
	merge m:1 itemc REP using "${temp}\ph.dta"
	drop if _merge == 2
	drop _merge
...
*==============================================================================		
	...
	save "${temp}\NISH2021_FOOD_CONS_LONG.dta", replace
