...
*	use "$raw/HCES_rawdata_final.dta", clear
	use "$raw/HCES_2015_2016_Expenditure_sample.dta", clear
...
/*	tostring RWQ11, gen(hid_1) format(%02.0f) 
	...
	tostring CQ11, gen(hid_1) format(%02.0f) 
	tostring CQ12, gen(hid_2) format(%02.0f) 
	tostring CQ13, gen(hid_3) format(%02.0f) 
	tostring CQ14, gen(hid_4) format(%01.0f) 
	tostring CQ15, gen(hid_5) format(%02.0f) 
	tostring CQ16, gen(hid_6) format(%02.0f) 
	tostring CQ17, gen(hid_7) format(%02.0f) 
	tostring CQ18, gen(hid_8) format(%06.0f)
...
*	label var QUANT   "annual quantity"
	label var ITEMC			"item code"
*	label var VALUE  "annual expenditure value"
...
*	sum WGT_WEIGHT
	...
	preserve 
		...
		save "$temp/hces_hh_data_2015.dta", replace  
	restore  
...
*==============================================================================				
	...
	*2.2 First round checking (missing/negative/zeros)
	 *2.2.1 Quantity (QUANT) and value (VALUE)
	 foreach v of varlist QUANTITY VALUE {
		di in y "checking `v'"
		qui count if `v' < 0
		...
		replace `v' = . if `v' == 0  //Zero2Miss: 3 QUANT, 1 VALUE
	 }
	 qui count if mi(QUANTITY) & mi(VALUE)
	 di in g "# of missing quantity and missing value: " in y r(N) //should be dropped
	 drop if mi(QUANT) & mi(VALUE)
	 *!!!!!!!DROP 1: 977 obs drop (missing QUANT and VALUE)!!!!!!!
	 drop if mi(QUANT)
	 *!!!!!!!DROP 2: xxx obs drop (missing QUANT)                 !!!!!!!
	 
	 qui count if mi(QUANT) & !mi(VALUE)
	 di in g "# of missing quantity, but NOT missing/zero value: " in y r(N)
	 qui count if mi(VALUE) & !mi(QUANT)
...
*==============================================================================	
	...
	*3.2 variable name adjustment
	ren (VALUE QUANT FOOD ITEMC MAJOR NETCAL)  ///
...
*==============================================================================
	preserve 
		...
		save "${temp}\HCES2015sample_FOOD_CONS.dta", replace
	restore
...
*		drop Foodexp
		drop dup_tag
		save "${temp}\HCES2015sample_NONFOOD_CONS.dta", replace
	restore
